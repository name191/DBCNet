...
class RandomRotation:
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if random.random() < self.p:
            # print(random_angle, "*******")
            img = TF.rotate(img, random_angle, TF.InterpolationMode.BILINEAR, self.expand, fill=[0.0])
            mask = TF.rotate(mask, random_angle, TF.InterpolationMode.NEAREST, self.expand, fill=[0.0])
        return img, mask
...
if __name__ == '__main__':
    ...
    #     # Pad((512, 512))
    # ])
    aug=get_val_augmentation((512,512))
